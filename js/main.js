...
// var edAppId = 'bc5cbaa0';
// var edAppKey = '381962b6de0bc353997fbbf9824d4794';

// "c579d9d3"
// "eddd63f4bfe096f4a9662f17302fd0fe"
...
function generateMeals() {
  Promise.all([generateBreakfast(), generateLunch(), generateDinner()])
    ...
    .catch(function (error) {
      $('#alert-modal-container').removeClass('form-none');
      $('#alert-text').text('Sorry, your meals failed to load. Please try again in 60 seconds.');
      $('#alert-ok-btn').attr('disabled', 'disabled').addClass('cursor-not-allowed').removeClass('hover:bg-gray-50').text('Try again');
      setTimeout(function () {
        $('#alert-ok-btn').removeAttr('disabled').removeClass('cursor-not-allowed').addClass('hover:bg-gray-50');
      }, 60000);
      $('#alert-ok-btn').on('click', function () {
        $('#alert-modal-container').addClass('form-none');
        generateMeals();
...
function renderWeekdayMeals(recipeImage, data, recipeName, recipeUrl, recipeCalories, recipeProtein, recipeFats, recipeCarbs, refreshBtn, calories) {
  ...
  localStorage.setItem(calories, Math.floor(data.hits[recipeRandomNum].recipe.calories / data.hits[recipeRandomNum].recipe.yield));
  $(refreshBtn).on('click', function () {
    renderWeekdayMeals(recipeImage, data, recipeName, recipeUrl, recipeCalories, recipeProtein, recipeFats, recipeCarbs, refreshBtn, calories);
    callRemainingFunction();
  });
}
